...
import { ChallengeResult, DecisionProof, ExecutionAnalysis, TraceDiff, challengeDecisionProof, getAnalysis, getDecisionProof, getFeaturedRun, getRun, getTraceDiff, recordActionApproval, startAgentRun } from "@/lib/api-client";
...
function Inspector({ event, analysis, proof, challenge, diff, disabledEvidence, approvalMessage, onSelectEvidence, onToggleEvidence, onApproval, onBranch }: { event: ExecutionEvent; analysis: ExecutionAnalysis | null; proof: DecisionProof | null; challenge: ChallengeResult | null; diff: TraceDiff | null; disabledEvidence: string[]; approvalMessage: string; onSelectEvidence: (id: string) => void; onToggleEvidence: (key: string) => void; onApproval: (decision: "approved" | "rejected") => void; onBranch: (instruction: string) => void }) {
...
function Workspace({ initialRunId, baselineRunId }: { initialRunId: string; baselineRunId?: string }) {
  ...
  const [executionRun, setExecutionRun] = useState({ ...demoRun, status: "completed" as const });
